...
def main():
    ...
    df.iloc[:, 3] = df.iloc[:, 3].replace('-', '', regex=True)  # last name: remove dashes
    df.iloc[:, 3] = df.iloc[:, 3].replace(' ', '', regex=True)  # last name: remove spaces
    ...
    df = df.sort_values(by=df.columns[3]) # sort last name asec
